...
class SecurityValidator:
    ...
    async def run(self, state: QueryState, reporter: StageReporter | None = None) -> QueryState:
        ...
        start_iso = state.intent.time_range.start.isoformat() if state.intent and state.intent.time_range else None
        end_iso = state.intent.time_range.end.isoformat() if state.intent and state.intent.time_range else None
        ...
        if reporter:
            await reporter.stage_completed(
                "security_validation",
                {
                    ...
                    "checks_passed": ["ast_readonly", "time_window_enforced", "limit_enforced"],
                    "warnings": reasons,
                },
            )
